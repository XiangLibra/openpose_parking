...
FLIP_INDICES = [(0,3),(1,2)]
FLIP_INDICES_PAF = [(0,2)]

# and annot['area'] > MIN_AREA
def check_annot(annot):
    return annot['num_keypoints'] >= MIN_KEYPOINTS  #and not annot['iscrowd'] == 1
...
def get_heatmap(coco, img, keypoints, sigma):
    ...
    # out_map_part = out_map[18]
    #                 # channel_np = channel_np.squeeze(0)


 
    # out_paf_uint8 = (out_map_part * 255).astype('uint8')


    # cv2.imshow('Image', out_paf_uint8)
    # cv2.waitKey(0)
...
def get_paf(coco, img, keypoints, sigma_paf, variable_width):
        ...
        for i in range(len(BODY_PARTS)):
            ...
            if keypoints_person[part[0], 2] > 0 and keypoints_person[part[1], 2] > 0:
                ...
                if l>1e-2:
                    ...
                    # print("outpaf_0")
                    # out_paf=out_pafs[i, 1]
                    # print(out_paf.shape)
                    # print("outpaf_1")
                    # print(len(out_pafs[i, 1]))
    ...
    # out_paf = out_pafs[16,0]
                    # channel_np = channel_np.squeeze(0)


#      # 将数据转换为 8 位无符号整数
#     out_paf_uint8 = (out_pafs * 255).astype('uint8')
